export type RestOrderType = 'market' | 'limit' | 'iceberg' | 'stop-loss' | 'take-profit' | 'stop-loss-limit' | 'take-profit-limit' | 'trailing-stop' | 'trailing-stop-limit' | 'settle-position';

export type RestOrderDirection = 'buy' | 'sell';

export type RestOpenOrder = {
    refid: string | null; // Referral order transaction id that created this order
    userref?: number | null; // user reference ID
    cl_ord_id?: string;
    ...
    trades?: string[]; // only present when `trades: true` is requested
};
